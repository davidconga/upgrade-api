    ...
    */
    ...
    public function calculateFare($request, $cflag=0){
        ...
        try{
            ...
            $ride_city_price = RideCityPrice::where('city_id', $request['city_id'])->where('ride_category', $request['ride_selected'])->where('ride_packages_id', $request['package_id'])->where('ride_delivery_vehicle_id', $request['service_type'])->first();
            ...
            if(!empty($location['errors'])){
                ...
                } else if($request['ride_selected'] == "RENTAL") {
                    $return_data['estimated_fare']=$this->applyNumberFormat(floatval($total)); 
                }
    ...
    public function applyPriceLogic($requestarr,$iflag=0){
        ...
        $ride_city_price = RideCityPrice::where('city_id', $requestarr['city_id'])->where('ride_category', $requestarr['ride_selected'])->where('ride_packages_id', $requestarr['package_id'])->where('ride_delivery_vehicle_id', $requestarr['service_type'])->first();
        ...
        if($ride_city_price != null) {
            if($ride_city_price->calculator == 'MIN') {
            ...
            } else if($ride_city_price->calculator == 'RENTAL') {
                ...
                {
                    //Hour Price
                    $extra_price += ($ride_city_price->minute*( ceil(($total_hours-$ride_city_price->hour)/60) ));
                }

                if($ride_city_price->distance < $total_kilometer)
                {
                    //Distance
                    $extra_price += ($ride_city_price->price*($total_kilometer-$ride_city_price->distance));
